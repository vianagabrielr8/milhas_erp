...
const Estoque = () => {
  ...
  // Agrupamento por Programa (COM ID)
    ...
    return filteredBalance.reduce((acc, item) => {
      ...
      if (!acc[item.program_name]) {
        acc[item.program_name] = {
          programId: item.program_id, // <--- ID necessário para o link
          totalBalance: 0,
          totalInvested: 0,
          accounts: [],
        };
    ...
    <MainLayout>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-8">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <Plane className="h-4 w-4" /> Total em Estoque
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">{formatNumber(totalMiles)}</div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <Wallet className="h-4 w-4" /> Total Investido
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <TrendingUp className="h-4 w-4" /> CPM Médio Global
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-primary">{formatCPM(avgCpmGlobal)}</div>
            <p className="text-sm text-muted-foreground">custo por milheiro</p>
          </CardContent>
        </Card>
      </div>

      {/* Cards de Programas (CLICÁVEIS) */}
        ...
        {Object.entries(balanceByProgram).map(([programName, data]) => {
          ...
          return (
            <Card 
                ...
                onClick={() => navigate(`/estoque/${data.programId}`)} // <--- CLIQUE AQUI
            >
              <CardHeader className="bg-muted/30 pointer-events-none">
                <CardTitle className="flex items-center justify-between">
                  <span>{programName}</span>
                  <Badge variant="secondary">CPM: {formatCPM(programCpm)}</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4 pointer-events-none">
                <div className="mb-4 pb-4 border-b">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold">{formatNumber(data.totalBalance)}</span>
                  </div>
                  <div className="flex justify-between items-center mt-1">
                    <span className="text-muted-foreground">Investido:</span>
                    <span className="font-medium">{formatCurrency(data.totalInvested)}</span>
                  </div>
                </div>
                  ...
                  {data.accounts.map((account) => (
                    <div key={account.name} className="flex justify-between items-center p-2 rounded bg-muted/50">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">CPM: {formatCPM(account.avgCpm)}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold">{formatNumber(account.balance)}</div>
                        <div className="text-xs text-muted-foreground">{formatCurrency(account.invested)}</div>
                      </div>
                    </div>
                  ))}
                </div>
