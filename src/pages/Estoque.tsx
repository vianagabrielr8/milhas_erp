  ...
  return (
    <MainLayout>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-8">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Plane className="h-4 w-4" /> Total em Estoque</CardTitle></CardHeader>
          <CardContent><div className="text-3xl font-bold">{formatNumber(totalMiles)}</div><p className="text-sm text-muted-foreground">milhas disponíveis</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Wallet className="h-4 w-4" /> Total Investido</CardTitle></CardHeader>
          <CardContent><div className="text-3xl font-bold">{formatCurrency(totalInvested)}</div><p className="text-sm text-muted-foreground">valor em milhas</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><TrendingUp className="h-4 w-4" /> CPM Médio Global</CardTitle></CardHeader>
          <CardContent><div className="text-3xl font-bold text-primary">{formatCPM(avgCpmGlobal)}</div><p className="text-sm text-muted-foreground">custo por milheiro</p></CardContent>
        </Card>
      </div>

      {filteredExpiringMiles.length > 0 && (
        <Card className="mb-8 border-warning/50 bg-warning/5">
          <CardHeader><CardTitle className="text-lg flex items-center gap-2 text-warning"><AlertTriangle className="h-5 w-5" /> Milhas a Vencer (próximos 30 dias)</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-3">
              {filteredExpiringMiles.map((item) => (
                <div key={item.id} className="flex justify-between items-center p-3 rounded-lg bg-background border">
                  <div><span className="font-medium">{item.program_name}</span><span className="text-muted-foreground"> - {item.account_name}</span></div>
                  <div className="text-right"><div className="font-bold">{formatNumber(item.quantity || 0)} milhas</div><div className="text-sm text-warning">{item.expiration_date && format(new Date(item.expiration_date), 'dd/MM/yyyy', { locale: ptBR })}<span className="ml-1">({item.days_until_expiration} dias)</span></div></div>
                </div>
              ))}
            </div>
          </CardContent>
              ...
              <CardContent className="pt-4">
                ...
                <div className="space-y-3">
                  {data.accounts.map((account) => (
                    <div key={account.name} className="flex justify-between items-center p-2 rounded bg-muted/50">
                      <div><div className="font-medium text-sm">{account.name}</div><div className="text-xs text-muted-foreground">CPM: {formatCPM(account.avgCpm)}</div></div>
                      <div className="text-right"><div className="font-bold">{formatNumber(account.balance)}</div><div className="text-xs text-muted-foreground">{formatCurrency(account.invested)}</div></div>
                    </div>
                  ))}
                </div>
              </CardContent>
